...
class Decoder(nn.Module):
    def __init__(self, conv_dim=512):
        ...
        self.upsample = nn.Sequential(
            ...
            nn.InstanceNorm2d(conv_dim // 4, affine=True),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(conv_dim//4, conv_dim // 8, kernel_size=4, stride=2, padding=1, bias=False),
            nn.InstanceNorm2d(conv_dim // 8, affine=True),
            nn.ReLU(inplace=True),
            # nn.ReflectionPad2d(3),
            nn.Conv2d(conv_dim // 8, conv_dim // 8, kernel_size=3, padding=1),
            nn.Conv2d(conv_dim // 8, 2, kernel_size=1, padding=0),
...
class Tuning(nn.Module):
    ...
    def forward(self, agnostic, cloth):
        out_parse, softM_parse = self.bodyshape(agnostic)
        target_bshape = torch.argmax(softM_parse, dim=2)
        t_bshape = target_bshape.view(self.batch_size, 1, 256, 192)

        t_bshape =  F.interpolate(t_bshape, size=(16,12), mode='bilinear')
        t_bshape = F.interpolate(t_bshape, size=(256,192),mode='bilinear')
...
def get_opt():
    ...
    parser.add_argument("--name", default="PGP")
    ...
    parser.add_argument('--tensorboard_dir', type=str, default='tensorboard2', help='save tensorboard infos')
...
# opt = get_opt()
# tuning = Tuning(opt=opt)
# agnostic = torch.randn((1,19,256,192))
# cloth = torch.randn((1,3,256,192))
# tuning(agnostic,cloth)